...
namespace nemesis {
    ...
    const char driver::description[] = "description: Hey b*tch, this is Nemesis compiler and package manager";
    ...
    const char driver::help[] = "commands:\n"
                                ...
                                "    remove <lib>:            uninstalls a dependency library from current workspace\n"
    ...
    void driver::parse_arguments(int argc, char **argv)
    {
        ...
        if (argc == 1) {
            ...
            error("I've never heard of command `$` before, idiot.", argv[1]);
            exit_code_ = impl::exit::failure;
        }
        ...
        for (auto i = 2; i < argc; ++i) {
            if (std::strcmp("-help", argv[i]) == 0) {
                ...
                for (auto j = i + 1; j < argc; ++j) arguments_.push_back(argv[j]);
                break;
            }
            else if (argv[i][0] == '-') {
            ...
            else arguments_.push_back(argv[i]);
        }
        // check number of arguments passed to command
        check_command_arguments_count(command_, arguments_.size());
    }

    void driver::check_command_arguments_count(enum command command, unsigned count)
    {
        switch (command) {
    ...
    void driver::build() 
    {
        ...
        pm::manager manager = pm::manager::instance(diagnostic_publisher_, source_handler_);
        // error code for file system operations
        std::error_code code;
    ...
    void driver::add() 
    {
        ...
        // try to add dependency generating new lock file
        pm::lock lock;
        // if version is specified
        if (arguments_.size() > 1) lock = manager.add_dependency(manifest, pm::manager::lock_path, arguments_[0], arguments_[1]);
        // otherwise
