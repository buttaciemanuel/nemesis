...
namespace nemesis {
    namespace utf8 {
        namespace impl {
            ...
            constexpr interval wide_east_asian[] = {
            ...
            constexpr interval control_characters[] = {
            { 1, 32 },
            { 0x07f, 0x09F }
            };

            /**
             * This function searches a unicode value inside a table to find out
             * if it belongs to it
             */
            bool binary_search(const interval *set, int n, codepoint value)
            {
                int low = 0, high = n - 1;

                while (low <= high) {
                    int mid = (low + high) / 2;
                    if (value < set[mid].first) {
                        high = mid - 1;
                    }
                    else if (value > set[mid].last) {
                        low = mid + 1;
                    }
                    else {
                        return true;
            ...
            /**
             * This helper functions computes the width of a unicode character as column spaces.
             * Emoji are treated by default as wide east asian characters whose width is 2.
             * It supports unicode 10.0 at least.
             * Null character has width 0
             * Control characters (including newline) have width 1 (for rendering on screen of spaces)
             * Wide east-asian characters (even emoji) have width 2
             * Normal characters have width 1
             */
            int width(codepoint value)
            {
                if (binary_search(control_characters, sizeof(control_characters) / sizeof(*control_characters), value))
                    return 1;
                else if (binary_search(basic_zero, sizeof(basic_zero) / sizeof(*basic_zero), value) || 
                        binary_search(zero_width, sizeof(zero_width) / sizeof(*zero_width), value))
                    return 0;
                else if (binary_search(wide_east_asian, sizeof(wide_east_asian) / sizeof(*wide_east_asian), value))
                    return 2;
        ...
        int encode(codepoint value, byte *units)
        {
            ...
            if (value <= 0x7f) {
                ...
                units[0] = 0xc0 | (0x1f & (value >> 6));
                units[1] = 0x80 | (0x3f & value);
                units[2] = 0;
                size = 2;
            }
            else if (value <= 0xffff) {
                units[0] = 0xe0 | (0xf & (value >> 12));
                units[1] = 0x80 | (0x3f & (value >> 6));
                units[2] = 0x80 | (0x3f & value);
                units[3] = 0;
                size = 3;
            }
            else if (value <= 0x10ffff) {
                units[0] = 0xf0 | (0x7 & (value >> 18));
                units[1] = 0x80 | (0x3f & (value >> 12));
                units[2] = 0x80 | (0x3f & (value >> 6));
                units[3] = 0x80 | (0x3f & value);
                units[4] = 0;
                size = 4;
            }
            else {
                throw std::invalid_argument("utf8::encode(): unicode code point is not valid");
            }
        ...
        codepoint decode(const byte *units)
        {
            ...
            if ((units[0] & 0xf8) == 0xf0) {
                ...
                throw std::invalid_argument("utf8::decode(): character is not UTF-8 encoded");
            }

            return value;
        }

        int width(codepoint val) { return impl::width(val); }

        span::span(span::iterator begin, span::iterator end, bool owner) :
            span(begin.ptr_, static_cast<int>(end.ptr_ - begin.ptr_), owner)
        {
            // invalid range
            if (begin > end) {
                throw std::invalid_argument("utf8::span::span(): iterators order is invalid");
            }
        }

        span::span(const span& other)
        {
            // if the other is a owner, then makes this one a owner of a copy of the buffer
            if (other.owns_) {
                units_ = new byte[other.size_];
                std::copy(other.units_, other.units_ + other.size_, units_);
                size_ = other.size_;
                owns_ = true;
            }
            // neither of them is a owner so they share a reference to the same buffer
            else {
                units_ = other.units_;
                size_ = other.size_;
                owns_ = false;
        ...
        int span::size() const { return size_; }

        byte* span::data() const { return units_; }

        char* span::cdata() const { return reinterpret_cast<char *>(units_); }

        std::string span::string() const { return std::string(cdata(), static_cast<std::size_t>(size_)); }

        span::iterator span::find(codepoint character) const
        {
            auto it = begin();
        ...
        span span::subspan(int start, int end) const
        {
            if (end < start) {
                throw std::invalid_argument("utf8::span::subspan(): end index is less than start index");
            }

            int index = 0;
            byte *begin = units_;

            while (begin < units_ + size_ && index < start) {
                if ((*begin & 0xf8) == 0xf0) {
                    begin += 4;
                }
            ...
            if (begin == units_ + size_) {
                throw std::out_of_range("utf8::span::subspan(): character index is out of range");
            }

            byte *past_end = begin;

            while (past_end < units_ + size_ && index < end) {
                if ((*past_end & 0xf8) == 0xf0) {
                    past_end += 4;
                }
                else if ((*past_end & 0xf0) == 0xe0) {
                    past_end += 3;
                }
                else if ((*past_end & 0xe0) == 0xc0) {
                    past_end += 2;
                }
                else if ((*past_end & 0x80) == 0x0) {
                    ++past_end;
                }
            ...
            return span(begin, static_cast<int>(past_end - begin), owns_);
        }

        span span::subspan(iterator start, iterator end) const
        {
            if (start > end) {
                throw std::invalid_argument("utf8::span::subspan(): iterators order is invalid");
            }

            return span(start.ptr_, static_cast<int>(end.ptr_ - start.ptr_), owns_);
        }

        codepoint span::at(int index) const
        {
            int count = 0;
            byte *ptr = units_;

            while (ptr < units_ + size_ && count < index) {
                if ((*ptr & 0xf8) == 0xf0) {
                    ptr += 4;
                }
            ...
            if (ptr == units_ + size_) {
                throw std::out_of_range("utf8::span::at(): character index is out of range");
            }

            return decode(ptr);
        }

        codepoint span::operator[](int index) const { return at(index); }
        ...
        int span::width() const
        {
            int width = 0;
            for (codepoint c : *this) width += utf8::width(c);
            return width;
        }

        bool span::owns() const { return owns_; }

        span::iterator span::begin() const { return iterator(units_); }
        
        span::iterator span::end() const { return iterator(units_ + size_); }

        int span::compare(const span& other) const
        {
        ...
        bool span::operator==(const span& other) const { return compare(other) == 0; }

        span::iterator::iterator(byte *ptr) : ptr_{ptr} {}
        ...
        span::iterator span::iterator::operator+(int count) const
        {
            if (count < 0) {
                throw std::invalid_argument("utf8::span::iterator::operator+(int): increment must be a positive value");
            }
        ...
        codepoint span::iterator::operator*() const
        {
            return decode(ptr_);
        }
        ...
        span::builder::builder() : bytes_{} {}

        span::builder& span::builder::add(codepoint character)
        {
            byte units[4] = { 0x0 };
        ...
        span span::builder::build() const
        {
            return span(bytes_.data(), static_cast<int>(bytes_.size()), true);
        }
    }
}