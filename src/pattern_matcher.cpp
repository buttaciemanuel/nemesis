...
namespace nemesis {
    namespace ast {
        ...
        ast::pointer<ast::expression> pattern_matcher::match(const ast::expression* pattern, ast::pointer<ast::type> expected, const ast::expression* expression, ast::pointer<ast::expression> tree, struct result& result) const
        {
            ...
            case ast::kind::path_pattern_expression:
            {
                ...
                // 2) a constant name
                    ...
                    auto comparison = ast::create<ast::binary_expression>(source_range(), token::builder().artificial(true).kind(token::kind::equal_equal).lexeme(utf8::span::builder().concat("==").build()).build(), tree, from_pattern(pattern));
                ...
                if (!nemesis::types::assignment_compatible(expected, pattern->annotation().type)) {
                    ...
                    if (auto variant = std::dynamic_pointer_cast<ast::variant_type>(expected)) {
                        if (variant->contains(pattern->annotation().type)) {
                            ...
                            auto tag = token::builder().artificial(true).kind(token::kind::integer_literal).lexeme(utf8::span::builder().concat(typehash.data(), typehash.size()).build()).build();
                            auto right = ast::create<ast::identifier_expression>(source_range(), tag, ast::pointers<ast::expression>());
                    ...
                    if (auto structure = std::dynamic_pointer_cast<ast::structure_type>(pattern->annotation().type)) {
                        for (int i = 0; i < record_pattern->fields().size() && record_pattern->fields().at(i)->kind() != ast::kind::ignore_pattern_expression; ++i) {
            ...
            case ast::kind::labeled_record_pattern_expression:
            {
                ...
                if (!nemesis::types::assignment_compatible(expected, pattern->annotation().type)) {
                    ...
                    if (auto structure = std::dynamic_pointer_cast<ast::structure_type>(pattern->annotation().type)) {
                        for (int i = 0; i < record_pattern->fields().size(); ++i) {
        ...
        ast::pointer<ast::expression> pattern_matcher::from_pattern(const ast::expression* pattern) const
        {
            ...
            case ast::kind::path_pattern_expression:
                if (pattern->annotation().type && pattern->annotation().referencing) {
